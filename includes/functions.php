...
function get_social_platforms_meta(): array {
    return [
        ...
        'telegram' => [
            ...
            'hint'        => 'Telegram handle or direct link. Opens direct Telegram chat.',
            'badge'       => 'Direct Chat',
            'color'       => '#0088cc',
            ...
            'format_url'  => function($v) {
                if (str_starts_with($v, 'http')) return $v;
                return 'https://t.me/' . ltrim(trim($v), '@');
            }
        ],
        'discord' => [
            'name'        => 'Discord',
...
function get_user_social_links(array $user): array {
    ...
    foreach ($platforms as $key => $meta) {
        ...
        $active[$key] = [
            ...
            'color'       => $meta['color'],
            'icon'        => $meta['icon'],
            'direct_chat' => in_array($key, ['whatsapp', 'telegram', 'discord', 'contact_email', 'facebook'], true)
        ];
    }
